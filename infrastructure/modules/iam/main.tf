# -----------------------------------------------------------------------------
# IAM Module — ECS Roles + GitHub OIDC
# -----------------------------------------------------------------------------

data "aws_caller_identity" "current" {}

# ---- ECS Task Execution Role ----
# Used by ECS agent to pull images, push logs, and read secrets
...
# Allow reading specific secrets from Secrets Manager
...
# EFS access for bulk file storage
...
# S3 access for Phase 2 bulk upload migration
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3:::${var.project_name}-${var.environment}-bulk",
          "arn:aws:s3:::${var.project_name}-${var.environment}-bulk/*"
        ]
      }
    ]
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            "ecs:cluster" = "arn:aws:ecs:${var.aws_region}:${var.aws_account_id}:cluster/${var.project_name}-${var.environment}"
          }
        }
      },
      {
